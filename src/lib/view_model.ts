	...
	return paginatedOutfitStates.map(outfitStatesPage => {
		return outfitStatesPage.map((outfitState, i) => {
			...
			return {
				...
				pictureLink: getFacePicLink(outfitState.characterSlug, (outfitState.broken && !outfitState.unlocked) ? '' : (outfitState.nameSlug as string), outfitState.broken),
	...
	return paginatedCharacterStates.map(characterStatesPage => {
		return characterStatesPage.map((characterState, i) => {
			...
			return {
				...
				pictureLink: getFacePicLink(characterState.nameSlug, getHeaviestOutfitSlug(characterState), false),
				pictureHeight: width * viewPortWidth / viewPortHeight,
				weightInLbs,
